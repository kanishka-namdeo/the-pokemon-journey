...
function evoCondition(d) {
  ...
  /* 'use-item' is already said by the item bit above */
  if (d.trigger && d.trigger.name !== 'level-up' && d.trigger.name !== 'use-item') bits.push(String(d.trigger.name).replace(/-/g, ' '));
...
/* Result-affecting parts of a view, normalized the same way matchesQuery
   treats q — lets applyView skip the grid teardown on no-op keystrokes. */
function viewSignature(v) {
  if (!v) return null;
  return [v.type || '', v.gen || 0, v.sort || 'num', String(v.q || '').trim().toLowerCase()].join('\u0000');
}
function shouldRefilter(a, b) {
  return viewSignature(a) !== viewSignature(b);
}
...
const Dex = (function () {
  ...
  let lastView = null; /* null forces the first applyView to render */
  ...
  function renderTypes() {
    document.getElementById('dexTypes').innerHTML = deps.TYPES.map(t =>
      `<button class="dex-typechip" data-type="${t.name}" style="--tc:${t.color};color:${t.text || '#0F1220'}" aria-pressed="false">${t.name}</button>`).join('');
  }
  ...
  function wireControls() {
    ...
    /* Debounced: each keystroke only records the query; the grid rebuild
       waits for a 150 ms typing pause (applyView also no-ops on no-ops). */
    let searchTimer = 0;
  ...
  function applyView() {
    if (!list) return;
    if (!shouldRefilter(lastView, view)) return;
    lastView = { q: view.q, type: view.type, gen: view.gen, sort: view.sort };
  ...
  function clearSelection() {
    state.selId = 0;
    els.device.classList.remove('dex-detail-open');
  ...
  async function openDetail(id, optsIn) {
    ...
    state.selId = id;
    els.device.classList.add('dex-detail-open');
  ...
  function chip(name) {
    ...
    const text = t ? (t.text || '#0F1220') : '#0F1220';
    return `<span class="dex-typechip" style="--tc:${color};background:${color};color:${text}">${name}</span>`;
  }
  ...
  function renderDetail(r, opts) {
    ...
    /* Fallback learnsets (all methods at level 0) must not pose as LEVEL-UP rows. */
    const levelless = r.moves.rows.length > 0 && r.moves.rows.every(m => !m.level);
    const movesSub = (levelless ? 'MOVES' : 'LEVEL-UP MOVES') + ' · LATEST GAMES' +
      (r.moves.total > r.moves.rows.length ? ` (FIRST ${r.moves.rows.length} OF ${r.moves.total})` : ` (${r.moves.total} TOTAL)`);
    const movesNote = levelless ? '<p class="dex-movesnote">LEARNED VIA TM, EGG, AND TUTOR IN THE LATEST GAMES.</p>' : '';
    els.right.innerHTML =
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { TYPE_ORDER, TYPE_CHART, typeMatchup, FLAVOR_PREF, cleanFlavor, pickFlavor, dexIdFromUrl, evoCondition, flattenEvo, latestMoves, buildRecord, GEN_RANGES, genOf, matchesQuery, sortList, filterList, viewSignature, shouldRefilter, CACHE_KEY, createCache };
}
